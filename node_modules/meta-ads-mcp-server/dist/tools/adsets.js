...
import { getAccessToken, makeGraphApiCall, makeGraphApiPostCall, fetchNode, postNode, deleteNode, prepareParams, handleApiError, } from "../services/graph-api.js";
import { FieldsSchema, FilteringSchema, PaginationSchema, TimeRangeSchema, DatePresetSchema, DateFormatSchema, EffectiveStatusSchema, } from "../schemas/common.js";
const ADSET_FIELDS_DESC = "Fields per ad set. Common: id, name, account_id, campaign_id, status, effective_status, daily_budget, lifetime_budget, budget_remaining, bid_amount, bid_strategy, billing_event, optimization_goal, targeting, start_time, end_time, created_time, updated_time, pacing_type, destination_type";
export function registerAdSetTools(server) {
    server.registerTool("meta_ads_get_adset_by_id", {
...
Examples:
  ...
  - Use when: "What is the optimization goal and status of this ad set?"`,
    ...
    server.registerTool("meta_ads_get_adsets_by_ids", {
...
Examples:
  - Use when: "Get details for ad sets 23843211234567, 23843211234568, and 23843211234569"`,
        ...
        try {
            ...
            const params = prepareParams({ access_token: token, ids: adset_ids.join(",") }, { fields, ...(date_format ? { date_format } : {}) });
    ...
    server.registerTool("meta_ads_get_adsets_by_adaccount", {
...
Returns:
  Object with data (ad set array) and paging. Use meta_ads_fetch_pagination_url with paging.next for more results.`,
    ...
    }, async ({ act_id, fields, filtering, date_preset, time_range, updated_since, effective_status, date_format, limit, after, before, }) => {
        try {
            const token = getAccessToken();
    ...
    server.registerTool("meta_ads_get_adsets_by_campaign", {
...
Returns:
  Object with data (ad set array) and paging. Use meta_ads_fetch_pagination_url with paging.next for more results.`,
    ...
    }, async ({ campaign_id, fields, filtering, effective_status, date_format, limit, after, before, }) => {
        try {
            const token = getAccessToken();
            const url = `${FB_GRAPH_URL}/${campaign_id}/adsets`;
            const params = prepareParams({ access_token: token }, { fields, filtering, effective_status, date_format, limit, after, before });
            const data = await makeGraphApiCall(url, params);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
                structuredContent: data,
...
Args:
  ...
  - attribution_spec (object[]): e.g., [{event_type:"CLICK_THROUGH", window_days:7}].`,
        ...
        try {
            ...
            if (args.dsa_beneficiary !== undefined)
                params.dsa_beneficiary = args.dsa_beneficiary;
            if (args.dsa_payor !== undefined)
                params.dsa_payor = args.dsa_payor;
            if (args.destination_type !== undefined)
            ...
            if (args.targeting !== undefined)
                params.targeting = JSON.stringify(args.targeting);
...
Note: frequency_control_specs is immutable after creation.`,
        ...
        try {
            ...
            if (args.billing_event !== undefined)
                params.billing_event = args.billing_event;
            if (args.start_time !== undefined)
                params.start_time = args.start_time;
            if (args.end_time !== undefined)
                params.end_time = args.end_time;
    ...
    server.registerTool("meta_ads_delete_adset", {
...
Irreversible — prefer meta_ads_pause_adset if you may want to resume later.`,
    ...
    server.registerTool("meta_ads_pause_adset", {
    ...
    server.registerTool("meta_ads_resume_adset", {
