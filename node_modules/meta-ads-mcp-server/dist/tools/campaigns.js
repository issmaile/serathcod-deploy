...
export function registerCampaignTools(server) {
    server.registerTool("meta_ads_get_campaign_by_id", {
...
Examples:
  ...
  - Use when: "What is the objective and status of my campaign?"`,
    ...
    server.registerTool("meta_ads_get_campaigns_by_adaccount", {
  ...
  Object with data (campaign array) and paging. Use meta_ads_fetch_pagination_url with paging.next for more results.`,
        inputSchema: z
            ...
            special_ad_categories: z
                ...
                .optional()
                .describe("Filter by special ad categories: EMPLOYMENT, HOUSING, CREDIT, ISSUES_ELECTIONS_POLITICS, NONE"),
            objective: z
                .array(z.string())
                .optional()
                .describe("Filter by objective: APP_INSTALLS, BRAND_AWARENESS, CONVERSIONS, EVENT_RESPONSES, LEAD_GENERATION, LINK_CLICKS, MESSAGES, PAGE_LIKES, POST_ENGAGEMENT, PRODUCT_CATALOG_SALES, REACH, VIDEO_VIEWS"),
    ...
    }, async ({ act_id, fields, filtering, date_preset, time_range, updated_since, effective_status, is_completed, special_ad_categories, objective, buyer_guarantee_agreement_status, date_format, include_drafts, limit, after, before, }) => {
        try {
...
Returns:
  { id: string } of the new campaign.`,
    ...
    }, async ({ act_id, name, objective, status, special_ad_categories, daily_budget, lifetime_budget, buying_type, bid_strategy, bid_cap, spend_cap, campaign_budget_optimization, use_adset_level_budgets, ab_test_control_setups, }) => {
        try {
            const params = {
                name,
                objective,
            ...
            if (use_adset_level_budgets) {
                ...
                if (lifetime_budget !== undefined)
                    params.lifetime_budget = String(lifetime_budget);
            ...
            if (bid_cap !== undefined)
                params.bid_cap = String(bid_cap);
            if (spend_cap !== undefined)
                params.spend_cap = String(spend_cap);
...
Returns:
  { success: true } or the updated campaign object.`,
    ...
    }, async ({ campaign_id, name, status, objective, special_ad_categories, daily_budget, lifetime_budget, bid_strategy, bid_cap, spend_cap, campaign_budget_optimization, adset_budgets, }) => {
        try {
            const params = {};
            if (name !== undefined)
                params.name = name;
            ...
            if (adset_budgets !== undefined) {
                params.adset_budgets = JSON.stringify(adset_budgets.map((b) => ({
                    adset_id: b.adset_id,
