...
import { getAccessToken, makeGraphApiCall, makeGraphApiPostCall, postNode, prepareParams, handleApiError, } from "../services/graph-api.js";
import { FieldsSchema, FilteringSchema, PaginationSchema, DateFormatSchema } from "../schemas/common.js";
const VALID_CROP_KEYS = [
    ["100x100", 100, 100],
    ["100x72", 100, 72],
    ["400x500", 400, 500],
    ["400x150", 400, 150],
...
function computeCropBox(srcW, srcH, kw, kh) {
...
Examples:
  ...
  - Use when: "Show all creatives with issues in act_123456"`,
        ...
        try {
            ...
            const params = prepareParams({ access_token: token }, { fields, effective_status, filtering, limit, after, before });
    ...
    server.registerTool("meta_ads_get_ad_creative_by_id", {
...
Examples:
  ...
  - Use when: "Get a larger thumbnail (300x200) for this creative"`,
        ...
        try {
            ...
            const params = prepareParams({ access_token: token }, {
                fields,
    ...
    server.registerTool("meta_ads_get_ad_creatives_by_ad_id", {
...
Examples:
  ...
  - Use when: "Get the image URLs and titles for all creatives on this ad"`,
        ...
        try {
            ...
            const params = prepareParams({ access_token: token }, { fields, date_format, limit, after, before });
            const data = await makeGraphApiCall(url, params);
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
                structuredContent: data,
    ...
    server.registerTool("meta_ads_compute_image_crops", {
...
Pass the returned image_crops dict to meta_ads_create_ad_creative.`,
        ...
        const keyMap = new Map(VALID_CROP_KEYS.map(([k, w, h]) => [k, [w, h]]));
        const crops = {};
        const warnings = [];
        for (const key of requested) {
            const dims = keyMap.get(key);
            if (!dims) {
                warnings.push(`'${key}' is not a valid Meta crop key. Valid keys: ${VALID_CROP_KEYS.map(([k]) => k).join(", ")}`);
                continue;
            }
        ...
        const result = {
            ...
            usage: "Pass image_crops directly to meta_ads_create_ad_creative inside object_story_spec.link_data.",
        };
        if (warnings.length)
...
Args:
  ...
  - url_tags (string, optional): UTM params appended to the URL`,
        inputSchema: z
            ...
            .refine((v) => Boolean(v.object_story_id) ||
            Boolean(v.object_story_spec) ||
            Boolean(v.asset_feed_spec) ||
            Boolean(v.image_hash) ||
            Boolean(v.video_id), {
            message: "Provide at least one of: object_story_id, object_story_spec, asset_feed_spec, image_hash, video_id.",
        }),
        annotations: {
            readOnlyHint: false,
            destructiveHint: false,
        ...
        try {
            ...
            if (args.object_story_id)
                params.object_story_id = args.object_story_id;
            ...
            if (!spec && !args.object_story_id) {
                        ...
                        content: [
                            {
                                type: "text",
                                text: JSON.stringify({
                                    error: "page_id is required when building a creative from simple fields (no object_story_id or object_story_spec).",
                                }, null, 2),
                            },
                        ],
                    };
                ...
                if (args.video_id) {
                    const video_data = { video_id: args.video_id };
                    if (args.message)
                        video_data.message = args.message;
                    if (args.headline)
                        video_data.title = args.headline;
                    if (args.link_url)
                        video_data.link_description = args.link_url;
            ...
            if (Object.keys(params).length === 0) {
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ error: "No update parameters provided (name or asset_feed_spec)." }, null, 2),
                        },
                    ],
                };
            }
