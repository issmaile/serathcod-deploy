...
export function getAccessToken() {
    ...
    throw new Error("Meta Ads access token is required. Provide it via --access-token argument or META_ADS_ACCESS_TOKEN environment variable.");
}
/**
 * Make a GET request to the Facebook Graph API.
 */
export async function makeGraphApiCall(url, params) {
 ...
 */
export async function makeGraphApiPostCall(url, params) {
 ...
 */
export async function makeGraphApiDeleteCall(url, params) {
 ...
 */
export async function postNode(path, options = {}) {
 ...
 */
export function prepareParams(baseParams, options) {
 ...
 */
export async function fetchNode(nodeId, options = {}) {
    const token = getAccessToken();
    const url = `${FB_GRAPH_URL}/${nodeId}`;
    const params = prepareParams({ access_token: token }, options);
 ...
 */
export async function fetchEdge(parentId, edgeName, options = {}) {
    const token = getAccessToken();
 ...
 */
export function buildInsightsParams(baseParams, opts) {
    const { fields, date_preset, time_range, time_ranges, time_increment, level, action_attribution_windows, action_breakdowns, action_report_time, breakdowns, default_summary, use_account_attribution_setting, use_unified_attribution_setting, filtering, sort, limit, after, before, offset, since, until, locale, } = opts;
